...
if __name__=='__main__':
    ...
    img, txt, labels,float_labels=0,0,0,0
    ...
    i_t_P,i_t_R=eva.pr_curve1(query_img_code,txt_code, query_S)
    t_i_P,t_i_R =eva.pr_curve1(query_txt_code,img_code, query_S)
    for i in range(i_t_P.shape[0]):
        print(i)
        print(i_t_P[i])
        print(i_t_R[i])

    for i in range(t_i_P.shape[0]):
        print(i)
        print(t_i_P[i])
        print(t_i_R[i])
